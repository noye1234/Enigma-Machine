...
//        }
    ...
    public String getCurrentRotorsPositionCompact(char ch){
        ...
        for (Integer rotorIndex : order) {
            if (ch=='(' || ch==')'){
                sb.append(rotors.get(rotorIndex).toString());
                sb.append(",");
            }
            else
                sb.append(rotors.get(rotorIndex).toStringWithoutChar());
                sb.append(ch);
        }
        sb.deleteCharAt(sb.length() - 1); // Remove last ch
        return sb.toString();
    ...
    public String getReflectorId() {
        return this.reflectors.getId();
    }


    public Map<Character, Character> getPlugboardPairs() {
        return this.plugboardPairs.getPlugboardPairs();
    }
}
